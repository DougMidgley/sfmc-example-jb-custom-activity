...
function requestedInteraction(payload) {
  ...
  const selectElement = document.querySelector("#discount-code");
  ...
  for (const a of payload.activities) {
    if (
      ...
      for (const inArgument of a.schema.arguments.execute.outArguments) {
        if (inArgument.discountCode) {
  ...
  if (selectElement.childElementCount == 0) {
    document.querySelector("#main-form").style.display = "hidden";
    document.querySelector("#warning").style.display = "block";
  } else {
    document.querySelector("#main-form").style.display = "block";
    document.querySelector("#warning").style.display = "hidden";
    ...
    if (selectedValue) {
      ...
      if (selectOption) {
        ...
        console.log(
          "Could not select value from list",
          `[value='${selectedValue}]'`
        );
      }
    }
...
function onDoneButtonClick() {
  ...
  for (const inArgument of activity.arguments.execute.inArguments) {
    if (inArgument.discount) {
      inArgument.discount = `{{Interaction.${option.value}.discount}}`;
    } else if (inArgument.discountCode) {
      inArgument.discountCode = `{{Interaction.${option.value}.discountCode}}`;
    }
  }
...
function setupEventHandlers() {
  document.querySelector("#done").addEventListener("click", onDoneButtonClick);
  document
...
function setupExampleTestHarness() {
  ...
  jb.ready = function () {
    ...
    jbSession.trigger(
      ...
      SampleInteraction.requestedInteractionDefaults
    );

    // Simulated the completion of "requestedInteraction"
    jbSession.trigger(
      "requestedInteraction",
      SampleInteraction.requestedInteraction
    );
